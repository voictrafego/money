"""Captura one-time (D-08) dos 104 tickers da B3 com o dado **SUJO** do jeito que ele
está hoje, e os congela num fixture YAML versionado — a evidência intocada do dado sujo,
contra a qual a Fase 9 mede o conserto, ticker a ticker. **NÃO regenerar nesta fase.**

Diferente de `capturar_snapshot_bancos.py` (referência de FORMA) em três pontos duros:

  1. **Universo:** os 104 tickers de `data/ticker_map.json` (não 4 bancos hardcoded).
  2. **Degradação por ticker (SAN-06, never-raise):** um ticker que falhe (montagem devolvendo
     None, exceção, 404 do Yahoo, ausência de preço) NÃO aborta a captura. Congela o que existir
     e registra a ausência no bloco `falhas:`. O MRFG3 (404 no Yahoo, virou MBRF) é congelado com
     as séries da CVM completas e `market_cap: null`. O script só devolve 1 se NENHUM ticker foi
     capturado.
  3. **Zero R$ derivado (D-05/D-07):** NÃO toca a camada de veredito/valuation e não carimba
     nenhum R$ por ticker (nem valor justo, nem motor, nem arquétipo). Insumo bruto, não veredito.

`ano_base`/`anos_historico` vêm de `config.yaml` (leitura, nunca escrita). O snapshot congela
`market_cap` e `splits` (não só o preço) para o baseline não piscar (EQTL3 a 0,5% do limiar do
SAN-01; splits mutáveis do Yahoo).

Uso: PYTHONPATH=src .venv/bin/python scripts/capturar_snapshot_sujo.py
"""
...
DATA_BASE = "2026-07-14"
CONFIG_PATH = os.path.join(ROOT, "config.yaml")
TICKER_MAP_PATH = os.path.join(ROOT, "data", "ticker_map.json")
DESTINO = os.path.join(ROOT, "tests", "fixtures", "snapshot_sanidade_2026-07-14.yaml")
...
def main() -> int:
    ...
    print(
        f"[snapshot gravado em {DESTINO}] — {ok} capturados, {len(falhas)} em falhas:",
        file=sys.stderr,
    )
