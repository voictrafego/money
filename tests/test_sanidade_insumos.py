...
def test_o_filtro_estreito_da_cvm_perde_o_jcp(monkeypatch):
    """SAN-03: no BRSR6 (JCP perdido) o filtro amplo enxerga MAIS que o estreito; no ITUB4
    (grande banco que escapa por acidente) os dois são iguais. Asserts de comparação entre
    dois campos — sem nenhuma constante numérica (BLIND-04a fica longe)."""
    monkeypatch.setattr(build.prices, "coletar_mercado", lambda t: _dm_stub(t))
    ...
    ub = brsr.ultimo_ano()
    assert brsr.proventos_filtro_amplo[ub] > brsr.dividendos[ub]  # o JCP perdido aparece
    ui = itub.ultimo_ano()
    assert itub.proventos_filtro_amplo[ui] == itub.dividendos[ui]  # escapa por acidente
