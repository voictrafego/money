...
- [ ] **Phase 11: Crescimento / `g` (GROW)** - `g_cap = (1+π_ciclo)(1+PIB_real)−1 = 7,28%`; o teste de invariância à inflação vira verde AQUI
...
**Success Criteria** (what must be TRUE):
  ...
  2. Existe um teste de **invariância à inflação** que é hoje `xfail(strict=True)` e **falha se passar**: `V` varia < 2% sob choque de **+300 bps** aplicado *simultaneamente* a `rf` e `g_cap`. Ele **é a Doença 1 escrita como código** e vira verde sozinho na Fase 11.
...
**Success Criteria** (what must be TRUE):
  1. **O teste de invariância à inflação (BLIND-02) vira VERDE sozinho** — o `xfail(strict=True)` é removido porque o código passou a satisfazê-lo, não porque alguém o afrouxou. `V` varia < 2% sob +300 bps em `rf` e `g_cap` simultaneamente. **A Doença 1 está metade curada e isso é medível.**
